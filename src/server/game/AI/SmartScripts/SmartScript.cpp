...
void SmartScript::ProcessAction(SmartScriptHolder& e, Unit* unit, uint32 var0, uint32 var1, bool bvar, const SpellInfo* spell, GameObject* gob)
{
    ...
    switch (e.GetActionType())
    {
        ...
        case SMART_ACTION_SUMMON_CREATURE:
        {
            ...
            if (targets)
            {
                ...
                for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
                {
                    ...
                    z += e.target.z;
                    o += e.target.o;
                    if (Creature* summon = object->SummonCreature(e.action.summonCreature.creature, x, y, z, o, (TempSummonType)e.action.summonCreature.type, e.action.summonCreature.duration))
            ...
            if (Creature* summon = object->SummonCreature(e.action.summonCreature.creature, e.target.x, e.target.y, e.target.z, e.target.o, (TempSummonType)e.action.summonCreature.type, e.action.summonCreature.duration))
        ...
        case SMART_ACTION_MOVE_TO_POS:
        {
            ...
            if (e.action.MoveToPos.RunPos == 1)
                ENSURE_AI(SmartAI, me->AI())->SetRun(true);
            else if (e.action.MoveToPos.RunPos == 2)
                ENSURE_AI(SmartAI, me->AI())->SetRun(false);
            ...
            else
            {
                ...
                if (e.action.MoveToPos.distance)
                {
                    x -= e.action.MoveToPos.distance * std::cos(me->GetOrientation());
                    y -= e.action.MoveToPos.distance * std::sin(me->GetOrientation());
                }

                me->GetMotionMaster()->MovePoint(e.action.MoveToPos.pointId, x, y, target->GetPositionZ());
            }
            break;
        }
...
ObjectList* SmartScript::GetTargets(SmartScriptHolder const& e, Unit* invoker /*= NULL*/)
{
    ...
    switch (e.GetTargetType())
    {
        ...
        case SMART_TARGET_VEHICLE_PASSENGER:
        {
            if (me)
            {
                if (Vehicle* tempVehicle = me->GetVehicleKit())
                    if (Unit* vpassenger = tempVehicle->GetPassenger(e.target.vehiclePassenger.seatId))
                        l->push_back(vpassenger);

                if (e.target.vehiclePassenger.checkVehicle)
                    if (Vehicle* tempVehicle = me->GetVehicle())
                        if (Unit* vpassenger = tempVehicle->GetPassenger(e.target.vehiclePassenger.seatId))
                            l->push_back(vpassenger);
            }
            break;
...
void SmartScript::ProcessEvent(SmartScriptHolder& e, Unit* unit, uint32 var0, uint32 var1, bool bvar, const SpellInfo* spell, GameObject* gob)
{
    ...
    switch (e.GetEventType())
    {
        ...
        case SMART_EVENT_SUMMONER_MISSING_BUFF:
        {
            if (!me->ToTempSummon())
                return;
            ...
            if (summoner->HasAura(e.event.missingSummonerBuff.spell))
                return;

            ProcessTimedAction(e, e.event.missingSummonerBuff.repeatMin, e.event.missingSummonerBuff.repeatMax, summoner);
            break;
        }
        //no params
